...
## Contexto do Projeto via Obsidian

Antes de começar o TDD, busque contexto nos docs do vault para entender o que já existe:

```bash
obsidian search query="<tema do TDD>" limit=5
obsidian read file="manifesto"      # visão geral, stack, ERPs suportados
obsidian read file="entidades"      # models existentes (evita recriar)
obsidian read file="fluxos"         # fluxos existentes a referenciar
obsidian read file="funcionalidades" # features já implementadas
```

Fallback: `mcp__obsidian-docs__read_file(path="docs/manifesto.md")`

**Wikilinks no TDD:** ao mencionar entidades, fluxos ou features do projeto nas seções narrativas do documento, use wikilinks Obsidian: `[[backend/entidades#Model|Model]]`, `[[fluxos#Fluxo]]`, `[[funcionalidades#Feature]]`. Nunca dentro de blocos de código ou tabelas de schema.
